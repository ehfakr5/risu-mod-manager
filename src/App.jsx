...
import { processModFiles, readFileAsJson, parseOriginalItems } from './utils/fileHandler'
import { mergeModsIntoOriginal, validateMergeResult, downloadCharxFile } from './utils/jsonMerger'
import { extractAndDownloadMod } from './utils/extractorUtils'

const MasterCheckbox = ({ modFiles, currentSection, onSelectAll, onDeselectAll }) => {
    const checkboxRef = useRef(null)

    // 현재 섹션의 mod들 필터링
    const filteredMods = currentSection === 'all'
        ? modFiles
        : modFiles.filter(mod => mod.section === currentSection)

    const selectedCount = filteredMods.filter(mod => mod.selected).length
    const totalCount = filteredMods.length
...
// remove_object 계산을 위한 헬퍼 함수
const calculateRemovedItems = (mods, originalItems = []) => {
    const removedItems = []
    const allItems = [...originalItems, ...mods]

    mods.forEach(mod => {
        if (mod.metadata?.remove_object && Array.isArray(mod.metadata.remove_object)) {
            mod.metadata.remove_object.forEach(pathToRemove => {
                const itemToRemove = allItems.find(item => item.name === pathToRemove)
                if (itemToRemove && !removedItems.find(r => r.name === pathToRemove)) {
                    removedItems.push({
                        ...itemToRemove,
                        id: `removed-${itemToRemove.id}`,
                        isRemoved: true,
                        removedBy: mod.name,
                        selected: false
                    })
...
function App() {
        ...
        if (file) {
            try {
                ...
                if (result.success) {
                    ...
                    // 원본 파일의 lorebook과 regex를 내부적으로만 저장 (화면에는 표시하지 않음)
                    const originalItems = parseOriginalItems(result.data)

                    // 기존 모드들에 대해 remove_object 재계산
                    setModFiles(prev => {
                        const existingMods = prev.filter(mod => !mod.isOriginal && !mod.isRemoved)
                        const removedItems = calculateRemovedItems(existingMods, originalItems)
                        return [...existingMods, ...removedItems]
                    })

                    // 원본 항목들을 상태로 저장 (병합 시 사용)
                    setOriginalJson(prevState => ({
                        ...prevState,
    ...
    const handleModFilesSelect = async (files) => {
        setUploadErrors([])

        try {
            const { results, errors } = await processModFiles(files)

            if (results.length > 0) {
                setModFiles(prev => {
                    const existingMods = prev.filter(mod => !mod.isRemoved)

                    // 새 mod들과 기존 mod들을 병합
                    const allNewMods = [...existingMods, ...results]

                    // metadata.order 기준으로 정렬 (내림차순)
                    // 같은 파일 내의 mod들은 원래 순서 유지를 위해 안정 정렬 사용
                    const sortedMods = allNewMods.sort((a, b) => {
                    ...
                    // remove_object로 삭제될 항목들을 감지하여 추가
                    const removedItems = calculateRemovedItems(sortedMods, originalJson?.originalItems || [])

                    return [...sortedMods, ...removedItems]
                })
            }
            ...
            setUploadErrors([`모드 파일 처리 오류: ${error.message}`])
        }
    }
    ...
    const handleSelectAll = () => {
        ...
        const filteredIds = filteredMods.map(mod => mod.id)

        setModFiles(prev =>
            prev.map(mod =>
                filteredIds.includes(mod.id)
                    ? { ...mod, selected: true }
                    : mod
            )
        )
    ...
    const selectedModsCount = modFiles.filter(mod => mod.selected && !mod.isRemoved).length

    // 선택된 mod에서 사용하는 슬롯 목록 추출 (같은 슬롯명은 병합)
    const getUsedSlots = () => {
        const selectedMods = modFiles.filter(mod => mod.selected && !mod.isRemoved)
        const slotMods = selectedMods.filter(mod => mod.section === 'slot')
        ...
        slotMods.forEach(mod => {
            ...
            const modName = mod.data.name || mod.name

            if (!slotGroups[slotname]) {
                slotGroups[slotname] = []
                separatorMap[slotname] = separator
                modNamesMap[slotname] = []
            }

            // 같은 슬롯의 경우 separator가 다르면 첫 번째 것 사용
            if (mod.data.content && Array.isArray(mod.data.content)) {
                slotGroups[slotname].push(...mod.data.content)
            }

            if (!modNamesMap[slotname].includes(modName)) {
                modNamesMap[slotname].push(modName)
            }
        })
    ...
    // Helper function: ID 목록에서 특정 prefix로 시작하는 인덱스를 역순 정렬하여 반환
    const getIndicesToDelete = (ids, prefix) => {
        return ids
        ...
        try {
            ...
            if (originalJson.isRisuModule) {
                if (updatedData.risuModule && updatedData.risuModule.lorebook) {
                    const lorebookEntriesToDelete = getIndicesToDelete(selectedOriginalItems, 'risu-lorebook-')
                    lorebookEntriesToDelete.forEach(index => {
                        updatedData.risuModule.lorebook.splice(index, 1)
                    })
                }
                if (updatedData.risuModule && updatedData.risuModule.regex) {
                    const regexEntriesToDelete = getIndicesToDelete(selectedOriginalItems, 'risu-regex-')
                    regexEntriesToDelete.forEach(index => {
                        updatedData.risuModule.regex.splice(index, 1)
                    })
                }
            } else {
                // 일반 캐릭터 카드인 경우
                if (updatedData.data) {
                    ...
                    if (updatedData.data.character_book && updatedData.data.character_book.entries) {
                        const lorebookEntriesToDelete = getIndicesToDelete(selectedOriginalItems, 'lorebook-')
                        lorebookEntriesToDelete.forEach(index => {
                    ...
                    if (updatedData.data.assets) {
                        const assetEntriesToDelete = getIndicesToDelete(selectedOriginalItems, 'asset-')
    ...
    const handleExtractSelected = async () => {
        ...
        try {
            ...
            const filename = `extracted_mod_${timestamp}.zip`

            // 선택된 항목들을 mod 형식으로 변환하고 ZIP 파일로 다운로드
            const result = await extractAndDownloadMod(originalJson, selectedOriginalItems, filename)

            if (result.success) {
                alert(`✅ 추출 완료! ${selectedOriginalItems.length}개 항목이 mod 형식으로 추출되었습니다.`)
                setSelectedOriginalItems([]) // 선택 상태 초기화
                setUploadErrors([])
            } else {
    ...
    const handleMergeAndDownload = async () => {
        if (!originalJson || selectedModsCount === 0) {
            return
        }
        ...
        try {
            ...
            const downloadResult = await downloadCharxFile(
                mergedData,
                filename,
                modFiles.filter(mod => mod.selected && !mod.isRemoved),
    ...
    return (
        <div className="min-h-screen bg-gray-900 p-6">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-2 gap-6 mb-6">
                    ...
                    <div className="bg-gray-800 rounded-xl border border-gray-700 p-5 shadow-sm min-h-[200px] flex flex-col">
                        ...
                        <div className="flex-1 flex items-center">
                            {originalJson ? (
                                ...
                                <div
                                    className={`w-full rounded-lg border-2 border-dashed p-6 text-center cursor-pointer transition-all duration-200 ${dragOver.original
                    ...
                    <div className="bg-gray-800 rounded-xl border border-gray-700 p-5 shadow-sm min-h-[200px] flex flex-col">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 className="text-base font-semibold text-white">모드 파일들</h3>
                        ...
                        <div className="flex-1 flex flex-col gap-3">
                                ...
                                <div className="w-full bg-gradient-to-r from-blue-900/20 to-indigo-900/20 rounded-lg border border-blue-700/50 p-4">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center">
                                            <span className="text-white font-bold text-sm">{modFiles.length}</span>
                                        </div>
                                        <div className="flex-1">
                                            <p className="text-sm font-medium text-blue-300">{modFiles.length}개 파일 업로드됨</p>
                            ...
                            {/* 드래그앤드랍 업로드 영역 (항상 표시) */}
                            <div
                                className={`w-full rounded-lg border-2 border-dashed text-center cursor-pointer transition-all duration-200 ${modFiles.length > 0 ? 'p-4' : 'p-6'
                                    } ${dragOver.mod
                                        ? 'bg-purple-900/20 border-purple-500'
                                        : 'bg-gray-700/50 border-gray-600 hover:bg-gray-700 hover:border-gray-500'
                                    }`}
                                onClick={() => document.getElementById('modFiles').click()}
                                onDragOver={(e) => handleDragOver(e, 'mod')}
                                onDragLeave={(e) => handleDragLeave(e, 'mod')}
                                onDrop={(e) => handleDrop(e, 'mod')}
                            >
                                <div className={`bg-gray-600 rounded-full flex items-center justify-center mx-auto ${modFiles.length > 0 ? 'w-8 h-8 mb-2' : 'w-12 h-12 mb-3'
                                    }`}>
                                    <svg className={`text-gray-300 ${modFiles.length > 0 ? 'w-4 h-4' : 'w-6 h-6'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                                    </svg>
                                </div>
                                <p className={`font-medium text-gray-300 mb-1 ${modFiles.length > 0 ? 'text-xs' : 'text-sm'
                                    }`}>
                                    {modFiles.length > 0 ? '추가 파일 업로드' : '여러 파일을 선택하세요'}
                                </p>
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                if (e.target.files && e.target.files.length > 0) {
                                    handleModFilesSelect(Array.from(e.target.files))
                                }
                            }}
                            className="hidden"
                        />
                ...
                <div className="mb-8">
                    <div className="bg-gray-800 rounded-lg border border-gray-700 shadow-sm">
                        <div className="px-4 py-3 border-b border-gray-700 bg-gray-700/50">
                            <h2 className="text-lg font-medium text-white">모드 파일 목록</h2>
                        </div>
                        ...
                        <div className="px-4 py-3 border-b border-gray-700">
                            <div className="flex gap-2 flex-wrap">
                                {[
                                    { key: 'all', label: '전체', count: modFiles.filter(d => !d.isRemoved).length },
                                    { key: 'lorebook', label: '로어북', count: modFiles.filter(d => d.section === 'lorebook' && !d.isRemoved).length },
                                    { key: 'asset', label: '에셋', count: modFiles.filter(d => d.section === 'asset' && !d.isRemoved).length },
                                    { key: 'slot', label: '슬롯', count: modFiles.filter(d => d.section === 'slot' && !d.isRemoved).length },
                                    { key: 'regex', label: '정규식', count: modFiles.filter(d => d.section === 'regex' && !d.isRemoved).length },
                                    { key: 'removed', label: '삭제됨', count: modFiles.filter(d => d.isRemoved).length }
                                ].map(({ key, label, count }) => (
                                    <button
                                        key={key}
                                        className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${currentSection === key
                                            ? key === 'removed'
                                ...
                                <div className="space-y-2 max-h-64 overflow-y-auto">
                                    {usedSlots.map((slot, index) => (
                                        <div key={index} className="bg-gray-700/50 rounded-lg p-3 border border-gray-600/50">
                                            <div className="mb-2">
                                                <span className={`text-sm font-mono px-2 py-1 rounded ${['<<lua>>', '<<toggle>>', '<<embedding>>'].includes(slot.slotname)
                                            ...
                                            <div className="flex flex-wrap gap-1 mb-2">
                                                {slot.modNames.map((modName, modIndex) => (
                                                    <span key={modIndex} className="text-xs text-gray-400 bg-gray-600/50 px-2 py-1 rounded">
                                                        {modName}
                                                    </span>
                                                ))}
                                            </div>
                                ...
                                <div className="text-sm text-gray-400">
                                    {selectedModsCount > 0 && (
                                        <span className="text-blue-400">
                                            {selectedModsCount}개 모드 선택됨
                                        </span>
                                    )}
                                </div>
                                <button
                                    className={`px-6 py-3 rounded-lg font-medium transition-all duration-200 flex items-center gap-2 ${!originalJson || selectedModsCount === 0
                                    ...
                                    disabled={!originalJson || selectedModsCount === 0}
                ...
                {/* Bottom Tables - Original and mod side by side */}
                <div className="grid grid-cols-2 gap-4">
                            ...
                            {originalJson && (
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all duration-200 flex items-center gap-1.5 ${selectedOriginalItems.length === 0
                                ...
                                {originalJson ? (
                                    <>
                                            ...
                                            <>
                                                ...
                                                {originalJson.data?.risuModule?.lorebook?.map((entry, index) => {
                                                    ...
                                                    return (
                                                        <div
                                                            key={itemId}
                                                            className={`grid grid-cols-12 gap-4 px-4 py-2 border-b border-gray-700 text-sm cursor-pointer hover:bg-gray-700 transition-colors ${selectedOriginalItems.includes(itemId) ? 'bg-gray-700' : 'bg-gray-800'
                                                                }`}
                                                ...
                                                {/* RisuAI 모듈의 regex 엔트리들 */}
                                                {originalJson.data?.risuModule?.regex?.map((entry, index) => {
                                                    const itemId = `risu-regex-${index}`
                                                    return (
                                                        <div
                                                            key={itemId}
                                                            className={`grid grid-cols-12 gap-4 px-4 py-2 border-b border-gray-700 text-sm cursor-pointer hover:bg-gray-700 transition-colors ${selectedOriginalItems.includes(itemId) ? 'bg-gray-700' : 'bg-gray-800'
                                                        ...
                                                        >
                                                            ...
                                                            <div className="col-span-8 truncate text-white">{entry.comment || `정규식 ${index + 1}`}</div>
                                                            <div className="col-span-3 text-gray-300">정규식</div>
                                                        </div>
                                                    )
                                                }) || []}
                                            </>
                                        ...
                                        {(originalJson.isRisuModule &&
                                            ...
                                            !originalJson.data?.risuModule?.regex?.length) && (
                                                <div className="text-center py-6 text-gray-400 text-sm">
                                                    로어북이나 정규식이 없습니다
                                                </div>
                                            )}
                    ...
                    <div className="bg-gray-800 rounded-lg border border-gray-700 shadow-sm">
                        <div className="px-4 py-3 border-b border-gray-700 bg-gray-700/50">
                            <h3 className="text-lg font-medium text-white">선택된 모드</h3>
                        </div>
                        <div className="bg-gray-700">
                            {/* Header */}
                            <div className="grid grid-cols-12 gap-4 px-4 py-2 border-b border-gray-600 text-sm font-medium text-gray-300">
                            ...
                            {/* Content */}
                            <div className="max-h-48 overflow-y-auto bg-gray-800">
                                {modFiles.filter(mod => mod.selected && !mod.isRemoved).length > 0 ? (
                                    modFiles.filter(mod => mod.selected && !mod.isRemoved).map((mod) => (
                                        <div key={mod.id} className="grid grid-cols-12 gap-4 px-4 py-2 border-b border-gray-700 text-sm bg-gray-800">
                                            <div className="col-span-7 truncate text-white">{mod.data.name || mod.name}</div>
                                            <div className="col-span-3 text-gray-300">
                                                {mod.section === 'lorebook' ? '로어북' :
                                                    mod.section === 'asset' ? '에셋' :
                                                        mod.section === 'slot' ? '슬롯' : mod.section}
                                            </div>
                                            <div className="col-span-2 text-center text-gray-300">
                                                {mod.section === 'lorebook' && mod.data.keys ? mod.data.keys.length :
                                                    mod.section === 'asset' && mod.data.content ? mod.data.content.length :
                                                        mod.section === 'slot' && mod.data.content ? mod.data.content.length : 0}
                                            </div>
                                        </div>
                                    ))
                                ) : (
